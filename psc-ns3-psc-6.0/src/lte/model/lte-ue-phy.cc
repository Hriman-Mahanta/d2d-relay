...
LteUePhy::LteUePhy (Ptr<LteSpectrumPhy> dlPhy, Ptr<LteSpectrumPhy> ulPhy)
  ...
  m_ueSdRsrqMeasurementsEnabled (false)
{
...
LteUePhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteUePhy")
    ...
    .AddTraceSource ("ReportUeSdRsrqMeasurements",
                     "Report Relay UE SD-RSRQ measurements (dB).",
                     MakeTraceSourceAccessor (&LteUePhy::m_reportUeSdRsrqMeasurements),
                     "ns3::LteUePhy::SdRsrqTracedCallback")
  ;
  return tid;
}
  ...
  if (m_rsrpSinrSampleCounter == m_rsrpSinrSamplePeriod)
    {
      ...
      // RSRP evaluated as coutaged received power among RBs
      double sum = 0.0;
      uint8_t rbNum = 0;
...
LteUePhy::ComputeAvgSinr (const SpectrumValue& sinr)
{
  ...
  //std::cout<<"Average SINR is"<< avrgSinr <<std::endl;
  return avrgSinr;
}
  ...
  if (msgType == LteSlDiscHeader::DISC_RELAY_ANNOUNCEMENT
      || msgType == LteSlDiscHeader::DISC_RELAY_RESPONSE)
    {
      ...
      //Measure SD-RSRQ
          ...
          double sd_rssi_W = (rssiSum / static_cast<double> (rbNum));
          double sd_rssi_dBm = 10 * log10 (1000 * (sd_rssi_W));
          
          std::cout<<"\nrbnum: "<<rbNum<<"\n";
          std::cout<<"\nRSSI: "<<sd_rssi_dBm<<"\n";

          // NS_ASSERT (rbNum == (*itPss).nRB);
          double sd_rsrq_dB = sd_rsrp_dBm/sd_rssi_dBm;
...
/*----------------------------------------Psddch RSRQ Measurements--------------------------------------------*/


void
LteUePhy::ReceivePsdchSdRsrq (Ptr<Packet> p, Ptr<SpectrumValue> psd, const std::vector<int> &rbs )
{
  NS_LOG_FUNCTION (this << m_rnti);
  ...
  if (msgType == LteSlDiscHeader::DISC_RELAY_ANNOUNCEMENT
      || msgType == LteSlDiscHeader::DISC_RELAY_RESPONSE)
    {
      ...
      //Store SD-RSRQ
      std::map <std::pair<uint64_t, uint32_t>, UeSdRsrqMeasurementsElement>::iterator
        itMeasMap = m_ueSdRsrqMeasurementsMap.find (std::pair<uint64_t, uint32_t> (relayUeId,serviceCode));
      if (itMeasMap == m_ueSdRsrqMeasurementsMap.end ())
        {
          NS_LOG_LOGIC (this << "First SD-RSRQ measurement entry");
          UeSdRsrqMeasurementsElement newEl;
          newEl.sdRsrqSum = sd_rsrp_W;
          newEl.sdRsrqNum = 1;
          m_ueSdRsrqMeasurementsMap.insert (std::pair< std::pair<uint64_t, uint32_t>,
                                                       UeSdRsrqMeasurementsElement> (std::pair<uint64_t, uint32_t> (relayUeId,serviceCode), newEl));
        }
      else
        {
          NS_LOG_LOGIC (this << "SD-RSRQ Measurement entry found... Adding values");
          (*itMeasMap).second.sdRsrqSum += sd_rsrq_dB;
          (*itMeasMap).second.sdRsrqNum++;
        }
    }
...
Time
LteUePhy::GetUeSdRsrqMeasurementsFilterPeriod ()
{
  Time filterPeriod;
  int32_t discPeriod;
...
void
LteUePhy::DoEnableUeSdRsrqMeasurements ()
{
  NS_LOG_FUNCTION (this);
  if (!m_ueSdRsrqMeasurementsEnabled)
    {
      Simulator::Schedule (GetUeSdRsrqMeasurementsFilterPeriod (), &LteUePhy::ReportUeSdRsrqMeasurements, this);
      m_ueSdRsrqMeasurementsEnabled = true;
    }
}

void
LteUePhy::DoDisableUeSdRsrqMeasurements ()
{
  NS_LOG_FUNCTION (this);
  if (m_ueSdRsrqMeasurementsEnabled)
    {
...
void
LteUePhy::ReportUeSdRsrqMeasurements ()
{
  NS_LOG_FUNCTION (this << m_rnti);
  if (m_ueSdRsrqMeasurementsEnabled)
    {
      //Perform the L1 filtering of all measured Relay UEs and service codes
      LteUeCphySapUser::UeSdRsrqMeasurementsParameters ret;
      std::map <std::pair<uint64_t, uint32_t>, UeSdRsrqMeasurementsElement>::iterator itMeasMap;
      for (itMeasMap = m_ueSdRsrqMeasurementsMap.begin (); itMeasMap != m_ueSdRsrqMeasurementsMap.end (); itMeasMap++)
        {
          //L1 filtering: linear average
          double avg_sd_rsrq_dB = (*itMeasMap).second.sdRsrqSum / static_cast<double> ((*itMeasMap).second.sdRsrqNum);
          //The stored values are in W, the report to the RRC should be in dBm
          //double avg_sd_rsrp_dBm = 10 * log10 (1000 * (avg_sd_rsrp_W));

          NS_LOG_INFO (this << " Relay UE Id " << (*itMeasMap).first.first
                            << "Service Code " << (*itMeasMap).first.second
                            << " averaged SD-RSRQ (dBm) " << avg_sd_rsrq_dB
                            << " number of measurements " << (*itMeasMap).second.sdRsrqNum );

          LteUeCphySapUser::UeSdRsrqMeasurementsElement newEl;
          newEl.m_relayId = (*itMeasMap).first.first;
          newEl.m_serviceCode = (*itMeasMap).first.second;
          newEl.m_sdRsrq = avg_sd_rsrq_dB;
          ret.m_ueSdRsrqMeasurementsList.push_back (newEl);

          m_reportUeSdRsrqMeasurements (m_rnti, (*itMeasMap).first.first,
                                        (*itMeasMap).first.second, avg_sd_rsrq_dB);
        }
      //Report to RRC
      m_ueCphySapUser->ReportUeSdRsrqMeasurements (ret);

      //Schedule next L1 filtering
      Simulator::Schedule (GetUeSdRsrqMeasurementsFilterPeriod (), &LteUePhy::ReportUeSdRsrqMeasurements, this);

      m_ueSdRsrqMeasurementsMap.clear ();
    }
}
