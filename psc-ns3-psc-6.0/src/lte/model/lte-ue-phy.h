...
public:
  ...
  virtual void ReceivePsdchSdRsrq (Ptr<Packet> p, Ptr<SpectrumValue> psd, const std::vector<int> &rbs);
...
private:
  ...
  */
  ...
  {
    double sdRsrqSum;   ///< Sum of SD-RSRP sample values in linear unit.
    uint16_t sdRsrqNum;  ///< Number of SD-RSRP samples.
  };
  ...
  std::map <std::pair<uint64_t,uint32_t>, UeSdRsrqMeasurementsElement> m_ueSdRsrqMeasurementsMap;
  ...
  bool m_ueSdRsrqMeasurementsEnabled;
  ...
  TracedCallback<uint16_t, uint64_t, uint32_t, double> m_reportUeSdRsrqMeasurements;
  ...
  Time GetUeSdRsrqMeasurementsFilterPeriod ();
  ...
  void DoEnableUeSdRsrqMeasurements ();
  ...
  void DoDisableUeSdRsrqMeasurements ();
  ...
  void ReportUeSdRsrqMeasurements ();
