...
private:
  ...
  void DoReportUeSdRsrqMeasurements (LteUeCphySapUser::UeSdRsrqMeasurementsParameters params);
...
//Sidelink related code
  ...
  struct SdRsrqMeasValue
  {
    double sdRsrq;   ///< Measured SD-RSRP in dBm.
    Time timestamp;   ///< The time stamp when the SD-RSRP was measured
  };
  ...
  std::map <std::pair<uint64_t,uint32_t>, SdRsrqMeasValue> m_storedSdRsrqMeasValues;
  /**
  ...
  void SaveSdRsrqMeasurements (uint64_t relayId, uint32_t serviceCode, double sdRsrq, uint16_t layer3FilterCoefficient);
