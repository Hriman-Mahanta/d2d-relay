...
LteUeRrc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::LteUeRrc")
    ...
    .AddAttribute ("MinSdRsrp",
                   ...
                   MakeDoubleChecker<double>())
    .AddAttribute ("MinSdRsrq",
                   "The minimum SD-RSRQ required to consider a Relay UE detectable [dBm]",
                   DoubleValue (-125),
                   MakeDoubleAccessor (&LteUeRrc::m_minSdRsrq),
                   MakeDoubleChecker<double>())
...
std::cout<<"---------------------------"<<std::endl;
 ...
 /* LteMacSapProvider::TransmitPduParameters params;
  ...
  std::cout<<"RNTI"<<m_rnti<<std::endl;*/

  uint32_t rv=discHeader.GetMyVariable ();
  NS_LOG_INFO("RV:   \n\n\n\n\n\n");
  std::cout<<"RV: "<<rv<<std::endl;

  std::vector<uint8_t> vec = discHeader.GetMyVector();
  for (auto element : vec) {
        std::cout<<0+element<<"\n";
    }

  
  uint8_t tm=discHeader.GetUtcBaseCounter ();
  ...
  uint64_t uinfo = discHeader.GetInfo ();
  std::cout<<"User Info "<<uinfo<<std::endl;

  uint32_t appcode = discHeader.GetApplicationCode ();
  std::cout<<"App code "<<appcode<<std::endl;
...
void
LteUeRrc::DoReportUeSdRsrqMeasurements (LteUeCphySapUser::UeSdRsrqMeasurementsParameters params)
{
  NS_LOG_FUNCTION (this << m_imsi);
  ...
  switch (m_state)
    ...
    case CONNECTED_REESTABLISHING:
      ...
      if (it != m_sidelinkConfiguration->m_slMap.end () && it->second.haveSib19)
        ...
        {
          NS_LOG_DEBUG ("SIB 19 not found. Unable to Save SD-RSRQ Measurements and perform Relay UE selection");
          return;
        }
      break;
  ...
  //Store values (Applying L3 filter if needed)
  std::vector <LteUeCphySapUser::UeSdRsrqMeasurementsElement>::iterator newMeasIt;
  for (newMeasIt = params.m_ueSdRsrqMeasurementsList.begin ();
       newMeasIt != params.m_ueSdRsrqMeasurementsList.end (); newMeasIt++)
    {
      NS_LOG_INFO (this << " UE IMSI " << m_imsi
                        << " reported measurements from Relay ID " << newMeasIt->m_relayId
                        << " Service Code " << newMeasIt->m_serviceCode
                        << " SD-RSRP " << newMeasIt->m_sdRsrq << " dBm ");

      SaveSdRsrqMeasurements (newMeasIt->m_relayId, newMeasIt->m_serviceCode, newMeasIt->m_sdRsrq, layer3FilterCoefficient);
    }
  ...
  for (newMeasIt = params.m_ueSdRsrqMeasurementsList.begin ();
       newMeasIt != params.m_ueSdRsrqMeasurementsList.end (); newMeasIt++)
    {
      std::map <std::pair<uint64_t,uint32_t>, SdRsrqMeasValue>::iterator storedMeasIt = m_storedSdRsrqMeasValues.find (std::pair<uint64_t,uint32_t> (newMeasIt->m_relayId,newMeasIt->m_serviceCode));
      if (storedMeasIt != m_storedSdRsrqMeasValues.end ()
          && (storedMeasIt->second.sdRsrq - qRxLevMin) > minHyst )
        {
          validRelays.insert (std::pair< std::pair<uint64_t,uint32_t>, double> (std::pair<uint64_t,uint32_t> (storedMeasIt->first.first, storedMeasIt->first.second), storedMeasIt->second.sdRsrq));
        }
    }
...
void
LteUeRrc::SaveSdRsrqMeasurements (uint64_t relayId, uint32_t serviceCode, double sdRsrq, uint16_t layer3FilterCoefficient)
{
  NS_LOG_FUNCTION (this << m_imsi << relayId << serviceCode << sdRsrq << layer3FilterCoefficient);

  std::map <std::pair<uint64_t,uint32_t>, SdRsrqMeasValue>::iterator storedMeasIt =
    m_storedSdRsrqMeasValues.find (std::pair<uint64_t,uint32_t> (relayId,serviceCode));
  if (storedMeasIt != m_storedSdRsrqMeasValues.end ())
    {
      NS_LOG_LOGIC (this << " This Relay UE was measured in the past");
      if (layer3FilterCoefficient != 0)
        {
          ...
          //Converting stored SD-RSRQ to linear units
          //double storedSdRsrq_db = std::pow (10.0,storedMeasIt->second.sdRsrq / 10.0) / 1000.0;
          double storedSdRsrq_dB = storedMeasIt->second.sdRsrq;
          double Fn_W;
          if (sdRsrq > m_minSdRsrq)
            {
              NS_LOG_LOGIC (this << " The fresh SD-RSRQ is above the minimum required, applying filter");
              //double newSdRsrq_dB = std::pow (10.0, sdRsrq / 10.0) / 1000.0;
              double newSdRsrq_dB = sdRsrq;
              ...
              //where nP is the number of 200 ms periods that have elapsed since last time
              //the quantity was filtered and stored
              double aSdRsrq = std::pow (0.5, layer3FilterCoefficient / 4.0); //TS 36.331 5.5.3.2
              double nP = (Simulator::Now ().GetMilliSeconds () -  storedMeasIt->second.timestamp.GetMilliSeconds ()) / 200;
              Fn_W = std::pow ((1 - aSdRsrq),nP) * storedSdRsrq_dB + aSdRsrq * newSdRsrq_dB;
              valid = true;
            }
          else
            {
              NS_LOG_LOGIC (this << " The fresh Relay UE S-RSRQ is below or equal to the minimum required... Ignoring measurement");
            }
          ...
          if (valid)
            {
              ...
              //If after the filtering, it decays below the minimum required, use the minimum required
              if (Fn_dBm <= m_minSdRsrq)
                {
                  NS_LOG_LOGIC (this << " The L3 filtered SD-RSRQ is below or equal to the minimum required, storing minimum required");
                  storedMeasIt->second.sdRsrq = m_minSdRsrq;
                  storedMeasIt->second.timestamp = Simulator::Now ();
                }
              else
                {
                  NS_LOG_LOGIC (this << " The L3 filtered SD-RSRQ is above the minimum required, storing it");
                  storedMeasIt->second.sdRsrq = Fn_dBm;
                  storedMeasIt->second.timestamp = Simulator::Now ();
                }
            }
        }
        ...
        {
          NS_LOG_LOGIC (this << " Not using L3 filtering, storing fresh value");
          storedMeasIt->second.sdRsrq = sdRsrq;
          storedMeasIt->second.timestamp = Simulator::Now ();
        }

      NS_LOG_INFO (this << " IMSI " << m_imsi << " measured Relay UE ID " << relayId
                        << " Service Code " << serviceCode
                        << " Fresh SD-RSRP " << sdRsrq << " Processed SD-RSRP " << storedMeasIt->second.sdRsrq);
    }
  else
    {
      NS_LOG_LOGIC (this << " First time measuring this Relay UE, storing first SD-RSRQ value");
      SdRsrqMeasValue v;
      v.sdRsrq = sdRsrq;
      v.timestamp = Simulator::Now ();
      m_storedSdRsrqMeasValues.insert (std::pair< std::pair<uint64_t,uint32_t>, SdRsrqMeasValue> (std::pair<uint64_t,uint32_t> (relayId,serviceCode), v));

      NS_LOG_INFO (this << " IMSI " << m_imsi << " measured Relay UE ID " << relayId
                        << " Service Code " << serviceCode
                        << " Fresh SD-RSRQ " << sdRsrq << " Processed SD-RSRP " << v.sdRsrq);
    }
}
