...
// void
// LteSlDiscHeader::SetMyVariable (uint8_t variable)
// {
//   m_myvar = variable;
// }
...
uint32_t
LteSlDiscHeader::GetMyVariable () const
{
  return m_myvar;
}

std::vector<uint8_t> LteSlDiscHeader::GetMyVector () const
{
  return m_myvector;
}
...
LteSlDiscHeader::Serialize (Buffer::Iterator start) const
{
  ...
  i.WriteU8(m_myvar);
  for (const uint8_t& element : m_myvector) {
        i.WriteU8(element);
    }
...
LteSlDiscHeader::Deserialize (Buffer::Iterator start)
{
  ...
  m_myvar = i.ReadU8();
  int size = m_myvector.size();
  m_myvector.clear();
  for (int j = 0; j < size; ++j) {
    m_myvector.push_back(i.ReadU8());
  }
