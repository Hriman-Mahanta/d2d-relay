...
void
ReportUeSdRsrqMeasurementsTrace (Ptr<OutputStreamWrapper> stream, uint16_t rnti, uint64_t relayId, uint32_t serviceCode, double sdRsrq)
{
  std::ostringstream oss;
  stream->GetStream ()->precision (6);
...
int main (int argc, char *argv[])
{
  ...
  *ReportUeSdRsrpMeasurementsStream->GetStream () << "time(s)\trnti\trelayId\tSC\tsdRsrp" << std::endl;
  ...
  //Tracing SD-RSRQ measurements in the Remote UEs
  Ptr<OutputStreamWrapper> ReportUeSdRsrpMeasurementsStream = ascii.CreateFileStream ("ReportUeSdRsrqMeasurementsTrace.txt");
  *ReportUeSdRsrpMeasurementsStream->GetStream () << "time(s)\trnti\trelayId\tSC\tsdRsrp" << std::endl;
  for (uint32_t ueDevIdx = 0; ueDevIdx < remoteUeDevs.GetN (); ueDevIdx++)
    {
      Ptr<LteUePhy> phy = remoteUeDevs.Get (ueDevIdx)->GetObject<LteUeNetDevice> ()->GetPhy ();
